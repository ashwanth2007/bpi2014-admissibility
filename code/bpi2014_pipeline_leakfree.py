...
# ---------------------------------------------------------------------------- diagnostics
# Mirrors of the two switches in bpi2014_data.py. They have to be mirrored because this file
# still carries its own copy of the loader rather than importing that one, which is itself a
# defect and is recorded as such; the attribution study below cannot run without them here.
# Both default OFF and neither is used by any published run.
#
...
#                        Detail_Incident.csv this work started from.
...
if LEGACY_DATES:
    ...
    print("  [diagnostic] BPI_LEGACY_DATES: incident timestamps parsed MONTH-first")
...
if TRUNCATE_ROWS:
    ...
    print("  [diagnostic] BPI_TRUNCATE=%d: incident file cut to %d rows"
...
# Rule 2, computed once over the whole log. THIS CALL IS LOAD-BEARING: without it the module
# level _CAUSAL_TE stays None, apply_group_encoding silently falls through to the train-rows
# path, and the whole run reports trainrows numbers while claiming exact Rule 2. That happened
# once on 2026-09-07 and produced a full re-run that changed nothing, with no error anywhere.
from causal_encoding import encode_causal as _enc
...
_theta, _cnt = _enc(_t_open, _t_res, g_all.values, y_all.values, alpha=SMOOTHING)
set_causal_te(pd.DataFrame({"Group_Breach_Rate_TE": _theta, "Group_Volume_TE": _cnt}, index=d.index))
print("Rule 2 mode: %s   causal cache populated: %s" % (RULE2_MODE, causal_te_available()))
