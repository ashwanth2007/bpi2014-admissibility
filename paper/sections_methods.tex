...
&\mathrm{BA} &= \tfrac{1}{2}\left(\mathrm{Recall}+\mathrm{Specificity}\right),
\label{eq:f1}
\end{align}
together with the Matthews correlation coefficient, which is the only one of these that
...
\SI{36.6}{\percent} minority we also report average precision, the area under the
...
so a drop from \num{0.8814} to \num{0.7859} is a loss of \num{0.0955} out of \num{0.3814}, which
is \SI{25.0}{\percent}. Second, every interval reported is a percentile bootstrap interval over
...
$\hat{\lambda} = \num{0.0191}\ \text{h}^{-1}$, \SI{95}{\percent} interval
$[\num{0.0161}, \num{0.0226}]$, on $n = \num{7041}$ cases. That is a half-life of
\num{36.3} hours.
...
$\hat{\tau}_0 = \num{-5.63}$ hours fitted on the same training period.
...
\num{50102} of \num{158300} evaluation cells, which is \SI{31.65}{\percent}. An explicit floor
...
\num{15828} cases into \num{69867} person-period rows, a factor of \num{4.41}, with a per-bin
resolution rate of \num{0.2260}. A gradient-boosted classifier estimates the discrete hazard
\begin{equation}
...
\qquad
P\bigl(\text{breach} \mid \text{alive at } t, \mathbf{x}\bigr) = \frac{S(\tau \mid \mathbf{x})}{S(t \mid \mathbf{x})},
\label{eq:survival}
\end{equation}
...
\begin{align}
\text{maximise} \quad & f_1(\mathbf{z}) = \tfrac{1}{n}\sum_{i}\sum_{j} z_{ij}\, \hat{p}^{\mathrm{ftr}}_{ij}
&& \text{(first-time-right resolution)} \label{eq:f1obj}\\
\text{minimise} \quad & f_2(\mathbf{z}) = \tfrac{1}{n}\sum_{i}\sum_{j} z_{ij}\, \hat{p}^{\mathrm{breach}}_{ij}
&& \text{(service level breach)} \label{eq:f2obj}\\
\text{minimise} \quad & f_3(\mathbf{z}) = \operatorname{sd}\bigl(\ell_1,\dots,\ell_m\bigr),
\quad \ell_j = \textstyle\sum_i z_{ij}
&& \text{(workload imbalance)} \label{eq:f3obj}\\
\text{maximise} \quad & f_4(\mathbf{z}) = \tfrac{1}{n}\sum_{i}\sum_{j} z_{ij}\, s_3(i,j)
&& \text{(expertise match)} \label{eq:f4obj}\\
\text{minimise} \quad & f_5(\mathbf{z}) = \tfrac{1}{n}\sum_{i}\sum_{j} z_{ij}\, \hat{d}_{ij}
&& \text{(expected delay before first touch)} \label{eq:f5obj}
\end{align}
subject to
\begin{align}
\text{C1:}\quad & \sum_{j} z_{ij} = 1 && \forall i \quad\text{(each incident assigned exactly once)}\\
\text{C2:}\quad & \sum_{i} z_{ij} \le \kappa_j && \forall j \quad\text{(group capacity)}\\
\text{C3:}\quad & z_{ij} \le a_j && \forall i,j \quad\text{(group availability)}\\
\text{C4:}\quad & z_{ij} \, s_3(i,j) \ge \eta \, z_{ij} && \forall i,j \quad\text{(minimum expertise eligibility)}
\end{align}
with $z_{ij}\in\{0,1\}$. Constraint C2 is enforced by a bounded repair operator rather than by
