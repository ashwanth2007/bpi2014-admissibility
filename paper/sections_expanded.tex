...
reassigned at least once take \num{8.95} times longer to resolve at the median, \num{14.53}
hours against \num{1.62}, and \SI{41.1}{\percent} of incidents are reassigned at least once.
...
\num{0.7859} at seventeen, a gain of \num{0.2243} AUC obtained without any inadmissible
...
explicitly that \textbf{BPI Challenge 2014 is not in the Teinemaa dataset list}, verified
against the paper itself. There is therefore no published same-task baseline on this log, so
the ranges reported in those benchmarks are used here as context and never as a head-to-head
comparison. \citet{fertig2026revisiting} revisit the field under foundation models and find
...
verified against that paper itself, so there is no published same-task baseline on this log and
the ranges reported in those benchmarks are used here as context and never as a head-to-head
comparison.
